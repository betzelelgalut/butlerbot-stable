...
class Googler:
	...
	def __googler(self, sInput, sSearchType = "what"):
		"Retrieve a list from Google for the string sInput."
		...
		try: result_page = urllib.urlopen(url)
			...
			print "ERROR: Could not connect to Google server."
			return {}
		...
		google_regex = re.compile(pattern)
		for line in lines:
			match = google_regex.search(line)
	...
	def respondTo(self, input):
		"""
		If input matches the form of a legal Google query (e.g. "Who is X?"),
		returns a random answer from the Google server.  None is returned
		if an error occurs.
		"""

		# Make sure the input is a Google-type question.
		...
		# a random selection.
		googs = self.__googler(result.group('query'), result.group('qtype'))
		if len(googs) != 0:
...
def main():
	print "Looking up meaning on Google.com..."
