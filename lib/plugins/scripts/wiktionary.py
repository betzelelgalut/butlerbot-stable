...
class wiktionary:
	def __init__(self):
		...
		self.__cacheFile = "var/cache/inet/wiktionary_cache"
	...
	def __googlisms(self, sInput, sSearchType = "what"):
		"Retrieve a list of wiktionary for the string sInput."
		...
		try: result_page = urllib.urlopen(url)
			...
			print "ERROR: Could not connect to Wiktionary server."
			return {}
		...
		wiktionary_regex = re.compile(pattern)
		for line in lines:
			match = wiktionary_regex.search(line)
	...
	def respondTo(self, input):
		"""
		If input matches the form of a legal wiktionary query (e.g. "Who is X?"),
		returns a random answer from the wiktionary server.  None is returned
		if an error occurs.
		"""
		...
		# a random selection.
		wikts = self.__wiktionaries(result.group('query'), result.group('qtype'))
		if len(wikts) != 0:
...
def main():
	print "Looking up meaning on wiktionary.org..."
	query = string.join(sys.argv[1:])
	wiktSearch = wiktionary()
	response = wiktSearch.respondTo(query)
	print response
